...
export function Header({ activeScreen, onNavigate, onResetFilters }: HeaderProps) {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      ...
      const pendingConversations = Number(kpiData?.statusSummary?.pending || 0);
      ...
      if (pendingConversations > 0) {
        items.push({
          id: `dashboard:pending:${pendingConversations}`,
          text: `${pendingConversations} hội thoại đang chờ xử lý`,
          type: "info",
          time: "Cập nhật theo dữ liệu Dashboard",
          status: "pending",
          targetScreen: "overview",
