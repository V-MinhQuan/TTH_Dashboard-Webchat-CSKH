...
const statusConfig: Record<SheetStatus, { bg: string; color: string; icon: typeof CheckCircle2 }> = {
  ...
  "Từ chối":            { bg: "#f1f5f9", color: "#64748b", icon: XCircle },
};
...
function normalizeSheetStatus(status: string): SheetStatus {
  if (status === "Đã duyệt") return "Đã duyệt";
  if (status === "Cần chỉnh sửa") return "Cần chỉnh sửa";
  if (status === "Từ chối" || status === "Bị từ chối") return "Từ chối";
  return "Chờ xử lý";
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", borderRadius: "16px", width: "520px", padding: "24px", boxShadow: "0 16px 48px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ backgroundColor: "#f8fafc", borderRadius: "10px", overflow: "hidden", marginBottom: "20px", border: "1px solid rgba(0,62,154,0.08)" }}>
          <table style={{ width: "100%", minWidth: "1320px", tableLayout: "fixed", borderCollapse: "collapse", fontSize: "12px" }}>
            <colgroup>
...
export function AddSheetModal({ prefillQuestion = "", prefillAnswer = "", initialRow, isEdit = false, onClose, onSave }: AddSheetModalProps) {
  const [question, setQuestion] = useState(initialRow?.question ?? prefillQuestion);
  const [answer, setAnswer] = useState(initialRow?.correctAnswer ?? prefillAnswer);
  const [topic, setTopic] = useState(initialRow?.topic ?? TOPICS[0]);
  const [source, setSource] = useState<SourceType>((initialRow?.source as SourceType) ?? SOURCES[0]);
  const [risk, setRisk] = useState<RiskLevel>(initialRow?.risk ?? "Thấp");
  ...
  const doSave = () => {
    const status = initialRow?.status ?? statusFromRisk(risk);
    onSave?.({ question, correctAnswer: answer, topic, source, risk, status, notes });
    if (isEdit) {
      toast.success("Đã cập nhật dữ liệu Sheet Chatbot");
    } else if (risk === "Cao") {
      ...
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 150, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ background: "#fff", borderRadius: "18px", width: "560px", maxHeight: "90vh", overflowY: "auto", padding: "28px", boxShadow: "0 16px 48px rgba(0,0,0,0.15)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <h3 style={{ fontSize: "16px", fontWeight: 700, color: NAVY, margin: 0 }}>{isEdit ? "Chỉnh sửa dữ liệu Sheet Chatbot" : "Thêm dữ liệu vào Sheet Chatbot"}</h3>
            <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(0,56,101,0.4)" }}><X size={18} /></button>
          </div>
          ...
          <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
            ...
            <button onClick={handleSave} disabled={!question.trim() || !answer.trim()} style={{ flex: 2, padding: "10px", borderRadius: "10px", border: "none", background: (!question.trim() || !answer.trim()) ? "#ccc" : NAVY, color: "#fff", cursor: (!question.trim() || !answer.trim()) ? "not-allowed" : "pointer", fontWeight: 600, fontSize: "13px" }}>
              {isEdit ? "Lưu thay đổi" : "Lưu vào Sheet Chatbot"}
            </button>
          </div>
        </div>
      </div>
...
export function SheetChatbot() {
  ...
  const currentUserName = user?.name || user?.username || (role === "manager" ? "Admin FLIC" : "Thu Trang");
  ...
  const [rows, setRows] = useState<SheetRow[]>(() => {
    if (typeof window !== "undefined") {
      ...
      if (saved) {
        try {
          return JSON.parse(saved).map((row: SheetRow) => ({
            ...row,
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === "flic_sheet_rows" && e.newValue) {
        try {
          setRows(JSON.parse(e.newValue).map((row: SheetRow) => ({
            ...row,
            status: normalizeSheetStatus(row.status),
          })));
        } catch (err) {
  ...
  const handleUpdateRow = (data: Omit<SheetRow, "id" | "addedAt" | "addedBy">) => {
    if (!editingRow) return;
    setRows(prev => prev.map(row => (
      row.id === editingRow.id
        ? { ...row, ...data, addedAt: row.addedAt, addedBy: row.addedBy }
        : row
    )));
    setEditingRow(null);
  };
  ...
  const updateRisk = (id: string, risk: RiskLevel) => {
    setRows(prev => prev.map(r => r.id === id ? { ...r, risk } : r));
    toast.success("Đã cập nhật mức rủi ro");
  };

  const statuses: SheetStatus[] = ["Chờ xử lý", "Đã duyệt", "Cần chỉnh sửa", "Từ chối"];
  const risks: RiskLevel[] = ["Thấp", "Trung bình", "Cao"];

  const kpiCounts = {
    total: filtered.length,
    pending: rows.filter(r => r.status === "Chờ xử lý").length,
    approved: rows.filter(r => r.status === "Đã duyệt").length,
    needsEdit: rows.filter(r => r.status === "Cần chỉnh sửa").length,
    rejected: rows.filter(r => r.status === "Từ chối").length,
  };

  return (
    <div style={{ padding: "24px", height: "100%", minHeight: 0, display: "flex", flexDirection: "column", gap: "20px" }}>
      {/* Header */}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px" }}>
        {[
          ...
          { label: "Cần chỉnh sửa", value: kpiCounts.needsEdit, color: "#7c3aed" },
        ].map(kpi => (
      ...
      {/* Table */}
      <div style={{ backgroundColor: "#fff", borderRadius: "16px", border: "1px solid rgba(0,62,154,0.07)", overflow: "hidden", flex: 1, minHeight: 0, display: "flex" }}>
        <div style={{ overflow: "auto", width: "100%", height: "100%" }}>
          <table style={{ width: "2000px", minWidth: "2000px", tableLayout: "fixed", borderCollapse: "collapse", fontSize: "12px" }}>
            <colgroup>
              <col style={{ width: "120px" }} />
              <col style={{ width: "160px" }} />
              <col style={{ width: "420px" }} />
            ...
            <thead>
              <tr style={{ backgroundColor: "#f8fafc", position: "sticky", top: 0, zIndex: 2 }}>
                {["Thời gian thêm", "Người thêm", "Câu hỏi", "Câu trả lời đúng", "Chủ đề", "Nguồn", "Mức rủi ro", "Trạng thái", "Hành động"].map(h => (
                  <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "rgba(0,62,154,0.5)", fontSize: "11px", letterSpacing: "0.04em", borderBottom: "1px solid rgba(0,62,154,0.07)", backgroundColor: "#f8fafc", whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {filtered.map(row => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: "12px 14px" }}>
                      <div style={{ color: "rgba(0,62,154,0.7)", lineHeight: 1.45, fontSize: "12px", whiteSpace: "normal" }}>{row.correctAnswer}</div>
                    </td>
                    <td style={{ padding: "12px 14px" }}>
                    ...
                    <td style={{ padding: "12px 14px", whiteSpace: "nowrap" }}>
                      <select
                        value={row.risk}
                        onChange={e => updateRisk(row.id, e.target.value as RiskLevel)}
                        style={{
                          fontSize: "10px",
      ...
      {showAddModal && (
        <AddSheetModal
          ...
          onClose={() => { setShowAddModal(false); setEditingRow(null); }}
