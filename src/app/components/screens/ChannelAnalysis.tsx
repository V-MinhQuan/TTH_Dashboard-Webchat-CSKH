...
const getInsightData = (ch: any) => {
  ...
  if (ch.ai_fail > 10) {
    observations.push(`Tỷ lệ AI phản hồi thất bại khá cao (${ch.ai_fail.toLocaleString()} ca, thành công ${aiRate}%).`);
  }
  if (ch.negative > 5) {
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={sortedRows} layout={chartType === "hbar" ? "vertical" : "horizontal"}>
        ...
        <Tooltip />
        {editValues.legend && <Legend iconSize={10} />}
        {keys.map((key, index) => <Bar maxBarSize={40} key={key} dataKey={key} stackId="a" fill={STATUS_COLORS[key] || PIE_COLORS[index % PIE_COLORS.length]} radius={index === keys.length - 1 ? [4, 4, 0, 0] : undefined} label={editValues.dataLabels ? { fontSize: 10 } : undefined} />)}
      </BarChart>
    </ResponsiveContainer>
  );
}
...
export function ChannelAnalysis({ filters, onFiltersChange, onNavigate }: ChannelAnalysisProps) {
  ...
  return (
    <div className="p-6">
      <FilterPanel filters={filters} onFiltersChange={handleApplyFilters} getExportData={getExportData} isLoading={loading} />
          ...
          <>
                ...
                >
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                    <div style={{ backgroundColor: "#f8fafc", borderRadius: "12px", padding: "14px 10px", textAlign: "center", border: "1px solid rgba(0,56,101,0.03)" }}>
                      ...
                      <div style={{ fontSize: "12px", color: "rgba(0,56,101,0.45)", marginTop: "4px", fontWeight: 400 }}>AI phản hồi thất bại</div>
                    </div>
                    <div style={{ backgroundColor: "#f8fafc", borderRadius: "12px", padding: "14px 10px", textAlign: "center", border: "1px solid rgba(0,56,101,0.03)" }}>
                      <div style={{ fontSize: "15px", fontWeight: 700, color: "#E05A1E" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
              ...
              <ChartCard
                ...
                baseFilters={appliedFilters}
                axisOptions={["Kênh"]}
                valueOptions={["AI phản hồi thành công", "AI phản hồi thất bại"]}
              >
            ...
            <div style={{ backgroundColor: "#fff", borderRadius: "20px", border: "1px solid rgba(0,56,101,0.08)", boxShadow: "0 2px 12px rgba(0,56,101,0.06)", overflow: "hidden", marginBottom: "20px" }}>
              <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(0,56,101,0.06)", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "16px", flexWrap: "wrap" }}>
                <div>
                  <h3 style={{ color: NAVY, fontSize: "14px", fontWeight: 700, margin: 0 }}>Heatmap: Số câu AI phản hồi thất bại theo kênh × chủ đề</h3>
                ...
                <table style={{ width: "100%", minWidth: `${heatmapMinWidth}px`, tableLayout: "fixed", borderCollapse: "collapse", fontSize: "12px" }}>
                  ...
                  <tbody>
                    {visibleChannels.map((ch) => (
                      <tr key={ch}>
                        <td className="flic-td-left" style={{ padding: "8px 16px", color: NAVY, fontWeight: 600, fontSize: "12px", whiteSpace: "nowrap" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                  <thead>
                    <tr style={{ backgroundColor: "#f8fafc" }}>
                      {["Kênh", "Hội thoại chờ", "AI phản hồi thất bại", "Hành động"].map((h, idx) => (
                        <th key={h} style={{ padding: "10px 14px", textAlign: "center", fontWeight: 600, color: "rgba(0,56,101,0.5)", fontSize: "10px", letterSpacing: "0.04em", borderBottom: "1px solid rgba(0,56,101,0.06)", whiteSpace: "nowrap" }}>
                          {h}
                        </th>
                      ))}
                    </tr>
